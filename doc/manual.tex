...
\bd
 ...
 \item[RESTART\_FILE] \kw{\{\la{\rm file name}\ra\}}
  \ko{RESTART\_FILE}\\
  Name of the restart file.\index{restart program run}\\
  default: \kw{RESTART}
\ed
%
...
%
\bd
 \item[SUBCELLS] \kw{\{\la{\rm real}\ra\}}
  \ko{SUBCELLS}\\
  The simulation cell is divided into subcells for the generation of the
  neighbor lists. The specified value defines the size of the subcells.
  Values between \kw{1.0} and \kw{2.0} show a good performance.\\
  default: \kw{1.5}
...
\bd
 ...
 \item[DENSITY\_GUESS,SCF\_GUESS,GUESS] \kw{\{ATOMIC,CORE,RESTART\}}
  ...
  matrix. For \kw{RESTART} a valid restart has to be supplied.
  \index{restart program run}\\
  default: \kw{ATOMIC}
 \item[OT]
  \ko{OT}~\\
