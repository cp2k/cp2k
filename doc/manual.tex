...
%          - PDF bookmarks added, some keyword updates (21.02.03,MK)
%
...
\def\ps@myheadings{%
 \def\@evenfoot{}%
 \def\@oddfoot{}%
 \def\@evenhead{\underline{\makebox[\textwidth][l]{{\rm\thepage}\hfill{\sl\leftmark}}}}%
 \def\@oddhead{\underline{\makebox[\textwidth][l]{{\sl\rightmark}\hfill{\rm\thepage}}}}%
 \let\@mkboth\markboth
 \def\sectionmark##1{\markboth{\ifnum\c@secnumdepth>\z@\thesection\quad\fi##1}{}}
 \def\subsectionmark##1{\markright{\ifnum\c@secnumdepth>\@ne\thesubsection\quad\fi##1}}}
...
\href{http://www.cscs.ch}{Swiss Center for Scientific Computing}, Switzerland\\
...
{\em CP2K User Manual (Release \releasenumber)}, Zurich (\number\year).\\[4mm]
This version of the manual was compiled by
Matthias Krack (\href{mailto:krack@cscs.ch}{krack@cscs.ch}).
\vfill
%
...
\ed
%
\subsection{Section \&IO}
\hypertarget{Section \&IO}{}\iso{\&IO}
%
...
\bd
 ...
 \item[EXCHANGE-CORRELATION-FUNCTIONAL,XC-FUNCTIONAL,FUNCTIONAL]~\\
  ...
  \ko{EXCHANGE-CORRELATION-\\FUNCTIONAL}\ko{XC-FUNCTIONAL}\ko{FUNCTIONAL}
  \karg{BLYP}\karg{BP86}\karg{LDA}\karg{PADE}\karg{PBE}\\
 ...
 \item[LSD]
  \ko{LSD}~\\
  A spin-polarized (unrestricted Kohn-Sham) calculation is requested.
 \item[MULTIPLICITY,MULTIP] \kw{\{\la{\rm integer}\ra\}}
  \ko{MULTIPLICITY}\ko{MULTIP}\\
  The multiplicity, i.~e.\ the number of unpaired electrons in the system plus
  one.\\
  default: \kw{1} for an even and \kw{2} for an odd number of electrons
\ed
%
\subsection{Section \&QS}
\iso{\&QS}
%
...
\bd
 ...
 \item[EPS\_EIGVAL] \kw{\{\la{\rm real}\ra\}}
  ...
  default: \kw{1.0E-5}
 \item[EPS\_JACOBI] \kw{\{\la{\rm real}\ra\}}
  \ko{EPS\_JACOBI}\\
  Pseudo diagonalization (Jacobi rotations) \cite{Stewart:1982} is used, if
  the maximum difference between the corresponding density matrix elements
  of two consecutive SCF iteration steps is smaller than the specified
  threshold value.\\
  default: \kw{0.0}
 \item[EPS\_SCF] \kw{\{\la{\rm real}\ra\}}
  ...
  steps.\\
  default: \kw{1.0E-5}
 \item[JACOBI\_THRESHOLD] \kw{\{\la{\rm real}\ra\}}
  \ko{JACOBI\_THRESHOLD}\\
  Threshold value for a Kohn-Sham matrix matrix element in the MO basis to
  perform a Jacobi rotations, if pseudo diagonalization is used.\\
  default: \kw{1.0E-7}
