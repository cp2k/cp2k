...
! *****************************************************************************
  SUBROUTINE dbcsr_get_2d_block_c(matrix,row,col,block,tr,found,&
       row_size, col_size)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_get_2d_block_c', &
...
!$     iw = omp_get_thread_num () + 1
  ...
  END SUBROUTINE dbcsr_get_2d_block_c
...
!> \param[out] col_size      (optional) logical column size of block
! *****************************************************************************
  SUBROUTINE dbcsr_get_block_c(matrix,row,col,block,tr,found,&
       row_size, col_size)
...
! *****************************************************************************
  SUBROUTINE dbcsr_get_tmp_block2d_c(matrix, row, col, block, tr,&
       found)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix
    INTEGER, INTENT(IN)                      :: row, col
    COMPLEX(kind=real_4), DIMENSION(:,:), POINTER         :: block
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_get_tmp_block2d_c', &
      routineP = moduleN//':'//routineN
...
!$     __LINE__, error=error)
!$  iw = omp_get_thread_num () + 1
    CALL btree_get_c (matrix%m%wms(iw)%mutable%m%btree_c,&
         make_coordinate_tuple(stored_row, stored_col),&
         data_block, found)
    ...
    ELSE
       ...
       tr = data_block%tr
    ENDIF
  END SUBROUTINE dbcsr_get_tmp_block2d_c
...
!   ---------------------------------------------------------------------------
    ...
    ELSE
       ...
       CALL pointer_c_rank_remap2 (pointer_any, rsize, csize,&
            lin_blk_p)
    ENDIF
...
!> \param[in,out] error        error
! *****************************************************************************
  SUBROUTINE dbcsr_set_block_pointer_1d_c (&
       matrix, pointer_any, row, col,&
       rsize, csize, main_tr, base_offset, buffer_tr, contiguous_pointers,&
       read_only, error)
    TYPE(dbcsr_obj), INTENT(IN)              :: matrix
    ...
    CHARACTER(len=*), PARAMETER :: &
      routineN = 'dbcsr_set_block_pointer_1d_c', &
      routineP = moduleN//':'//routineN
