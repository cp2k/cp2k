...
! *****************************************************************************
  SUBROUTINE mp_sum_d(msg,gid)
    ...
    msglen = 1
#if defined(__parallel)
    CALL mpi_allreduce(msg,res,msglen,MPI_DOUBLE_PRECISION,MPI_SUM,gid,ierr)
...
#if defined(__parallel)
    msglen = 1
    CALL mpi_allreduce(msg,res,msglen,MPI_DOUBLE_PRECISION,MPI_MAX,gid,ierr)
    IF ( ierr /= 0 ) CALL mp_stop( ierr, "mpi_allreduce @ "//routineN )
    msg = res
#endif
...
#if defined(__parallel)
    msglen = 1
    CALL mpi_allreduce(msg,res,msglen,MPI_DOUBLE_PRECISION,MPI_MIN,gid,ierr)
    IF ( ierr /= 0 ) CALL mp_stop( ierr, "mpi_allreduce @ "//routineN )
    msg = res
#endif
...
! *****************************************************************************
  SUBROUTINE mp_gather_dv(msg,msg_gather,root,gid)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'mp_gather_d]v', &
      routineP = moduleN//':'//routineN
