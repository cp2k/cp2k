...
! Transfer the correct parts of the input vector to the correct locations so we can do a local multiply
    CALL dbcsr_col_vec_to_rep_row_d(vec_in, work_col, work_row, fast_vec_col, error)
...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_d(work_col, work_row, fast_vec_row, error=error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_col_vec_to_rep_row_d(vec_in, rep_col_vec, rep_row_vec, fast_vec_col, error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_rep_row_to_rep_col_vec_d(rep_col_vec, rep_row_vec, fast_vec_row, fast_vec_col_add, error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE create_fast_col_vec_access_d(vec, fast_vec_access, error)
    TYPE(dbcsr_obj)                          :: vec
...
!$OMP END PARALLEL
    ...
    IF (col.GT.1) STOP "BUG in create_fast_col_vec_access_d"
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE create_fast_row_vec_access_d(vec, fast_vec_access, error)
    TYPE(dbcsr_obj)                          :: vec
    TYPE(fast_vec_access_type)               :: fast_vec_access
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
!$OMP END PARALLEL
    ...
    IF (row.GT.1) STOP "BUG in create_fast_row_vec_access_d"
...
!! parallel bits are locally added. The mp_sum magically creates the correct vector
    CALL dbcsr_rep_row_to_rep_col_vec_d(work_col, result_row, res_fast_vec_row, res_fast_vec_col, error)
