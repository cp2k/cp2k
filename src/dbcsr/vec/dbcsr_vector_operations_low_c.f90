...
! However, it restricts the sparsity of the vector as it assumes the sparsity pattern does not change
    ...
    CALL assign_row_vec_block_ptr_c(work_row, blk_map_row, error)
    CALL assign_col_vec_block_ptr_c(work_col, blk_map_col, error)

! Transfer the correct parts of the input vector to the correct locations so we can do a local multiply
    CALL dbcsr_col_vec_to_rep_row_c(vec_in, work_col, work_row, blk_map_col, error)
...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_c(work_col, work_row, blk_map_row, error=error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_col_vec_to_rep_row_c(vec_in, rep_col_vec, rep_row_vec, blk_map_col, error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_rep_row_to_rep_col_vec_c(rep_col_vec, rep_row_vec, blk_map_row, blk_map_col_add, error)
    TYPE(dbcsr_obj)                          :: rep_col_vec, rep_row_vec
    TYPE(block_ptr_c),DIMENSION(:), OPTIONAL     :: blk_map_col_add
    TYPE(block_ptr_c), ALLOCATABLE, &
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE assign_row_vec_block_ptr_c(row_vec, row_blk_ptr, error)  
    TYPE(dbcsr_obj)                          :: row_vec
    TYPE(block_ptr_c), DIMENSION(:)            :: row_blk_ptr
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
!$OMP END PARALLEL
  ...
  END SUBROUTINE assign_row_vec_block_ptr_c
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE assign_col_vec_block_ptr_c(col_vec, col_blk_ptr, error)
    TYPE(dbcsr_obj)                          :: col_vec
    TYPE(block_ptr_c), DIMENSION(:)            :: col_blk_ptr
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
!$OMP END PARALLEL
  ...
  END SUBROUTINE assign_col_vec_block_ptr_c
...
! It is important to note, that the input and result vector are sitributed differently (row wise, col wise respectively)
    ...
    CALL assign_row_vec_block_ptr_c(work_row, blk_map_row, error)
    CALL assign_col_vec_block_ptr_c(work_col, blk_map_col, error)
    CALL assign_row_vec_block_ptr_c(result_row, res_blk_map_row, error)
    CALL assign_col_vec_block_ptr_c(result_col, res_blk_map_col, error)
...
!! parallel bits are locally added. The mp_sum magically creates the correct vector
    CALL dbcsr_rep_row_to_rep_col_vec_c(work_col, result_row, res_blk_map_row, res_blk_map_col, error)
