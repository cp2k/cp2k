...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_z(work_col, work_row, blk_map_row, error=error)
...
! Hope this clarifies the idea
  ...
  SUBROUTINE dbcsr_rep_row_to_rep_col_vec_z(rep_col_vec, rep_row_vec, blk_map_row, blk_map_col_add, error)
    TYPE(dbcsr_obj)                          :: rep_col_vec, rep_row_vec
    TYPE(block_ptr_z),DIMENSION(:), OPTIONAL     :: blk_map_col_add
    TYPE(block_ptr_z), ALLOCATABLE, &
...
! It is important to note, that the input and result vector are sitributed differently (row wise, col wise respectively)
    ...
    CALL assign_row_vec_block_ptr_z(work_row, blk_map_row, error)
    CALL assign_col_vec_block_ptr_z(work_col, blk_map_col, error)
    CALL assign_row_vec_block_ptr_z(result_row, res_blk_map_row, error)
    CALL assign_col_vec_block_ptr_z(result_col, res_blk_map_col, error)
...
!$OMP END PARALLEL
    ...
    data_vec => dbcsr_get_data_p (result_row%m%data_area, coersion=CMPLX(0.0, 0.0, real_8))
...
!! parallel bits are locally added. The mp_sum magically creates the correct vector
    CALL dbcsr_rep_row_to_rep_col_vec_z(work_col, result_row, res_blk_map_row, res_blk_map_col, error)
