...
! *****************************************************************************
  SUBROUTINE dbcsr_matrix_colvec_multiply_low_z(matrix, vec_in, vec_out, alpha, beta, work_row, work_col, error)
    ...
    CASE(dbcsr_type_symmetric)
       CALL dbcsr_sym_matrix_vector_mult_z(matrix, vec_in, vec_out, alpha, beta, work_row, work_col, error)
...
! Transfer the correct parts of the input vector to the correct locations so we can do a local multiply
    CALL dbcsr_col_vec_to_rep_row_z(vec_in, work_col, work_row, fast_vec_col, error)
...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_z(work_col, work_row, fast_vec_row, error=error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_col_vec_to_rep_row_z(vec_in, rep_col_vec, rep_row_vec, fast_vec_col, error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_rep_row_to_rep_col_vec_z(rep_col_vec, rep_row_vec, fast_vec_row, fast_vec_col_add, error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE create_fast_col_vec_access_z(vec, fast_vec_access, error)
    TYPE(dbcsr_obj)                          :: vec
...
!$OMP END PARALLEL
    ...
    IF (col.GT.1) STOP "BUG in create_fast_col_vec_access_z"
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE create_fast_row_vec_access_z(vec, fast_vec_access, error)
    TYPE(dbcsr_obj)                          :: vec
    TYPE(fast_vec_access_type)               :: fast_vec_access
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
!$OMP END PARALLEL
    ...
    IF (row.GT.1) STOP "BUG in create_fast_row_vec_access_z"
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE assign_row_vec_block_ptr_z(row_vec, row_blk_ptr, error)  
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE assign_col_vec_block_ptr_z(col_vec, col_blk_ptr, error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_sym_matrix_vector_mult_z(matrix, vec_in, vec_out, alpha, beta, work_row, work_col, error)
    TYPE(dbcsr_obj)                          :: matrix, vec_in, vec_out
    COMPLEX(kind=real_8)                          :: alpha, beta
    TYPE(dbcsr_obj)                          :: work_row, work_col
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
!! parallel bits are locally added. The mp_sum magically creates the correct vector
    CALL dbcsr_rep_row_to_rep_col_vec_z(work_col, result_row, res_fast_vec_row, res_fast_vec_col, error)
