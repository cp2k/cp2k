...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_s(work_col, work_row, blk_map_row, error=error)
...
!> \param error ...
! *****************************************************************************
  SUBROUTINE dbcsr_rep_row_to_rep_col_vec_s(rep_col_vec, rep_row_vec, blk_map_row, blk_map_col_add, error)
    TYPE(dbcsr_obj)                          :: rep_col_vec, rep_row_vec
    TYPE(block_ptr_s),DIMENSION(:), OPTIONAL     :: blk_map_col_add
    TYPE(block_ptr_s), ALLOCATABLE, &
...
! It is important to note, that the input and result vector are sitributed differently (row wise, col wise respectively)
    ...
    CALL assign_row_vec_block_ptr_s(work_row, blk_map_row, error)
    CALL assign_col_vec_block_ptr_s(work_col, blk_map_col, error)
    CALL assign_row_vec_block_ptr_s(result_row, res_blk_map_row, error)
    CALL assign_col_vec_block_ptr_s(result_col, res_blk_map_col, error)
...
!$OMP END PARALLEL
    ...
    data_vec => dbcsr_get_data_p (result_row%m%data_area, coersion=0.0_real_4)
...
!! parallel bits are locally added. The mp_sum magically creates the correct vector
    CALL dbcsr_rep_row_to_rep_col_vec_s(work_col, result_row, res_blk_map_row, res_blk_map_col, error)
