...
! *****************************************************************************
  ...
  SUBROUTINE arnoldi_iram_d(arnoldi_data,error)
    TYPE(dbcsr_arnoldi_data)                 :: arnoldi_data
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
    ...
    COMPLEX(real_8), DIMENSION(:), ALLOCATABLE    :: work, tau
...
! *****************************************************************************
  ...
  SUBROUTINE gev_arnoldi_init_d (matrix, matrix_arnoldi, vectors, arnoldi_data, error)
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
   ...
   ! create a local data copy to store the vectors and make Gram Schmidt a bit simpler
    ...
    IF(control%max_ev_request)THEN
       CALL dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, -1.0_real_8, -ar_data%rho_scale, error)
    ELSE
       CALL dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, 1.0_real_8, -ar_data%rho_scale, error)
    END IF
  ...
  SUBROUTINE gev_build_subspace_d(matrix, vectors, arnoldi_data, error) 
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
    TYPE(m_x_v_vectors)                      :: vectors
    TYPE(dbcsr_arnoldi_data)                 :: arnoldi_data
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
! we set it to zero before and compute the distributed product only on the first col and then sum over the full grid
  ...
  SUBROUTINE gev_update_data_d(matrix, matrix_arnoldi, vectors, arnoldi_data, error)
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix_arnoldi
    TYPE(m_x_v_vectors)                      :: vectors
    TYPE(dbcsr_arnoldi_data)                 :: arnoldi_data
