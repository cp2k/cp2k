...
! *****************************************************************************
  ...
  SUBROUTINE gev_arnoldi_init_c (matrix, matrix_arnoldi, vectors, arnoldi_data, error)
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
   ...
   ! create a local data copy to store the vectors and make Gram Schmidt a bit simpler
    ...
    IF(control%max_ev_request)THEN
       CALL dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, -CMPLX(1.0, 0.0, real_4), -ar_data%rho_scale, error)
    ELSE
       CALL dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, CMPLX(1.0, 0.0, real_4), -ar_data%rho_scale, error)
    END IF
