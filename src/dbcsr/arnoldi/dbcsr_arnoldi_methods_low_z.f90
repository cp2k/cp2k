...
! *****************************************************************************
  ...
  SUBROUTINE arnoldi_iram_z(arnoldi_data,error)
    TYPE(dbcsr_arnoldi_data)                 :: arnoldi_data
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
! *****************************************************************************
  ...
  SUBROUTINE gev_arnoldi_init_z (matrix, matrix_arnoldi, vectors, arnoldi_data, error)
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
   ...
   ! create a local data copy to store the vectors and make Gram Schmidt a bit simpler
    ...
    IF(control%max_ev_request)THEN
       CALL dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, -CMPLX(1.0, 0.0, real_8), -ar_data%rho_scale, error)
    ELSE
       CALL dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, CMPLX(1.0, 0.0, real_8), -ar_data%rho_scale, error)
    END IF
...
! *****************************************************************************
  ...
  SUBROUTINE gev_build_subspace_z(matrix, vectors, arnoldi_data, error) 
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
    TYPE(m_x_v_vectors)                      :: vectors
    TYPE(dbcsr_arnoldi_data)                 :: arnoldi_data
    TYPE(dbcsr_error_type), INTENT(inout)    :: error
...
! *****************************************************************************
  ...
  SUBROUTINE gev_update_data_z(matrix, matrix_arnoldi, vectors, arnoldi_data, error)
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix
    TYPE(dbcsr_obj_type_p), DIMENSION(:)     :: matrix_arnoldi
    TYPE(m_x_v_vectors)                      :: vectors
    TYPE(dbcsr_arnoldi_data)                 :: arnoldi_data
