...
! However, it restricts the sparsity of the vector as it assumes the sparsity pattern does not change
    ...
    CALL assign_row_vec_block_ptr_s(work_row,blk_map_row)
    CALL assign_col_vec_block_ptr_s(work_col,blk_map_col)
...
! It is important to note, that the input and result vector are sitributed differently (row wise, col wise respectively)
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       IF(ASSOCIATED(blk_map_row(col)%ptr).AND.ASSOCIATED(blk_map_col(row)%ptr))THEN
          CALL sgemm('N','T',row_size,ncols,col_size,&
                     1.0_real_4,data_d,row_size,&
                     blk_map_row(col)%ptr,ncols,1.0_real_4,blk_map_col(row)%ptr,row_size)
       ELSE
          CALL sgemm('T','T',col_size,ncols,row_size,&
                     1.0_real_4,data_d,row_size,&
                     blk_map_row(row)%ptr,ncols,1.0_real_4,blk_map_col(col)%ptr,col_size)
       END IF
    END DO
...
! Perform the local multiply. Here it is obvious why the vectors are replicated on the mpi rows and cols
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       IF(ASSOCIATED(blk_map_row(col)%ptr).AND.ASSOCIATED(blk_map_col(row)%ptr))THEN
          CALL sgemm('T','N',ncols,col_size,row_size,&
                     1.0_real_4,blk_map_col(row)%ptr,row_size,&
                     data_d,row_size,&
                     1.0_real_4,blk_map_row(col)%ptr,ncols)
       ELSE
          CALL sgemm('T','T',ncols,row_size,col_size,&
                     1.0_real_4,blk_map_col(col)%ptr,col_size,&
                     data_d,row_size,&
                     1.0_real_4,blk_map_row(row)%ptr,ncols)
       END IF
    END DO
    CALL dbcsr_iterator_stop(iter)
...
! The same trick as described above with opposite direction
  ...
  SUBROUTINE assign_row_vec_block_ptr_s(row_vec,row_blk_ptr)  
  ...
  SUBROUTINE assign_col_vec_block_ptr_s(col_vec,col_blk_ptr)
