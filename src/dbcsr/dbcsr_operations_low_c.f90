...
!>
! *****************************************************************************
  SUBROUTINE dbcsr_trace_ab_c(matrix_a, matrix_b, trace, trans_a, trans_b, local_sum, error)
...
!   ---------------------------------------------------------------------------
  ...
  SUBROUTINE dbcsr_scale_by_vector_c(matrix_a, alpha, side, error)
    TYPE(dbcsr_obj), INTENT(INOUT)            :: matrix_a
  ...
  END SUBROUTINE dbcsr_scale_c_m

  !> \brief Interface for dbcsr_set
  SUBROUTINE dbcsr_set_c(matrix, alpha, error)
  ...
  SUBROUTINE dbcsr_add_alpha_c(matrix_a, matrix_b, alpha_scalar, beta_scalar, error)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix_a
    TYPE(dbcsr_obj), INTENT(IN)              :: matrix_b
    COMPLEX(kind=real_4), INTENT(IN)                      :: alpha_scalar
    COMPLEX(kind=real_4), INTENT(IN), OPTIONAL            :: beta_scalar
    TYPE(dbcsr_error_type), INTENT(INOUT)    :: error
    ...
    ENDIF
  END SUBROUTINE dbcsr_add_alpha_c
  !> \brief Interface for dbcsr_add THIS IS NOT NEEDED ANYMORE (alpha and beta ALWAYS REQUESTED)
  SUBROUTINE dbcsr_add_beta_c(matrix_a, matrix_b, alpha_scalar, beta_scalar, error)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix_a
    TYPE(dbcsr_obj), INTENT(IN)              :: matrix_b
    COMPLEX(kind=real_4), INTENT(IN), OPTIONAL            :: alpha_scalar
    ...
    ELSE
       CALL dbcsr_add_anytype(matrix_a, matrix_b,&
            beta_scalar=dbcsr_scalar(beta_scalar), error=error)
    ENDIF
  END SUBROUTINE dbcsr_add_beta_c
  ...
  SUBROUTINE dbcsr_set_diag_c(matrix, diag, error)
    TYPE(dbcsr_obj), INTENT(INOUT)            :: matrix
  ...
  SUBROUTINE dbcsr_get_diag_c(matrix, diag, error)
...
! *****************************************************************************
  SUBROUTINE dbcsr_add_on_diag_c(matrix, alpha_scalar, first_row, last_row, error)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix
    COMPLEX(kind=real_4), INTENT(IN)                      :: alpha_scalar
