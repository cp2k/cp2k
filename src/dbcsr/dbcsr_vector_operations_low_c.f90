...
! However, it restricts the sparsity of the vector as it assumes the sparsity pattern does not change
    ...
    CALL assign_row_vec_block_ptr_c(work_row,blk_map_row)
    CALL assign_col_vec_block_ptr_c(work_col,blk_map_col)
...
! The same trick as described above with opposite direction
  ...
  SUBROUTINE assign_row_vec_block_ptr_c(row_vec,row_blk_ptr)  
  ...
  SUBROUTINE assign_col_vec_block_ptr_c(col_vec,col_blk_ptr)
