...
! *****************************************************************************
  SUBROUTINE dbcsr_matrix_colvec_multiply_low_d(matrix,vec_in,vec_out,alpha,beta,work_row,work_col,error)
    ...
    CASE(dbcsr_type_symmetric)
       CALL dbcsr_matrixT_vector_mult_d(matrix,vec_in,vec_out,alpha,beta,work_row,work_col,.TRUE.,error)
       CALL dbcsr_matrix_vector_mult_d(matrix,vec_in,vec_out,alpha,1.0_real_8,work_row,work_col,error)
...
! However, it restricts the sparsity of the vector as it assumes the sparsity pattern does not change
    ...
    CALL assign_row_vec_block_ptr_d(work_row,blk_map_row)
    CALL assign_col_vec_block_ptr_d(work_col,blk_map_col)
...
! It is important to note, that the input and result vector are sitributed differently (row wise, col wise respectively)
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       IF(ASSOCIATED(blk_map_row(col)%ptr).AND.ASSOCIATED(blk_map_col(row)%ptr))THEN
          CALL dgemm('N','T',row_size,ncols,col_size,&
                     1.0_real_8,data_d,row_size,&
                     blk_map_row(col)%ptr,ncols,1.0_real_8,blk_map_col(row)%ptr,row_size)
       ELSE
          CALL dgemm('T','T',col_size,ncols,row_size,&
                     1.0_real_8,data_d,row_size,&
                     blk_map_row(row)%ptr,ncols,1.0_real_8,blk_map_col(col)%ptr,col_size)
       END IF
    END DO
...
! Perform the local multiply. Here it is obvious why the vectors are replicated on the mpi rows and cols
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       IF(ASSOCIATED(blk_map_row(col)%ptr).AND.ASSOCIATED(blk_map_col(row)%ptr))THEN
          CALL dgemm('T','N',ncols,col_size,row_size,&
                     1.0_real_8,blk_map_col(row)%ptr,row_size,&
                     data_d,row_size,&
                     1.0_real_8,blk_map_row(col)%ptr,ncols)
       ELSE
          CALL dgemm('T','T',ncols,row_size,col_size,&
                     1.0_real_8,blk_map_col(col)%ptr,col_size,&
                     data_d,row_size,&
                     1.0_real_8,blk_map_row(row)%ptr,ncols)
       END IF
    END DO
    CALL dbcsr_iterator_stop(iter)
...
! Convert the result to a column wise distribution
    CALL dbcsr_rep_row_to_rep_col_vec_d(work_col,work_row,blk_map_row,error)
...
! Hope this clarifies the idea
  ...
  SUBROUTINE dbcsr_rep_row_to_rep_col_vec_d(rep_col_vec,rep_row_vec,blk_map_row,error)
...
! The same trick as described above with opposite direction
  ...
  SUBROUTINE assign_row_vec_block_ptr_d(row_vec,row_blk_ptr)  
  ...
  SUBROUTINE assign_col_vec_block_ptr_d(col_vec,col_blk_ptr)
