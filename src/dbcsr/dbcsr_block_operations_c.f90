...
! *****************************************************************************
  PURE SUBROUTINE set_block_diagonal_c (block_data, value, d)
    COMPLEX(kind=real_4), DIMENSION(:), INTENT(OUT) :: block_data
    COMPLEX(kind=real_4), INTENT(IN)                :: value
    INTEGER, INTENT(IN)                :: d

!   ---------------------------------------------------------------------------

    CALL block_set_c (d, d, block_data, value, CMPLX(0.0, 0.0, real_4))
  END SUBROUTINE set_block_diagonal_c
...
!> \param beta ...
! *****************************************************************************
  PURE SUBROUTINE block_2d_set_c (m, n, blk, alpha, beta)
    INTEGER, INTENT(IN)                      :: m, n
    COMPLEX(kind=real_4), DIMENSION(m,n), INTENT(OUT)     :: blk
    COMPLEX(kind=real_4), INTENT(IN), OPTIONAL            :: alpha, beta

    CHARACTER(len=*), PARAMETER :: routineN = 'block_2d_set_c', &
      routineP = moduleN//':'//routineN

    INTEGER                                  :: i
    COMPLEX(kind=real_4)                                  :: my_alpha, my_beta
...
!   ---------------------------------------------------------------------------
    ...
    IF(PRESENT(alpha)) THEN
       my_alpha = alpha
    ELSE
       my_alpha = CMPLX(0.0, 0.0, real_4)
    ENDIF
    IF(PRESENT(beta)) THEN
       my_beta = beta
    ELSE
       my_beta = CMPLX(0.0, 0.0, real_4)
    ENDIF
    blk(:,:) = my_beta
    IF(m.EQ.n) THEN
       FORALL (i = 1:m)
...
!> \param beta ...
! *****************************************************************************
  PURE SUBROUTINE block_set_c (m, n, blk, alpha, beta)
    INTEGER, INTENT(IN)                      :: m, n
    COMPLEX(kind=real_4), DIMENSION(m*n), INTENT(OUT)     :: blk
    COMPLEX(kind=real_4), INTENT(IN), OPTIONAL            :: alpha, beta
...
!   ---------------------------------------------------------------------------
    ...
    CALL block_2d_set_c (m, n, blk, alpha, beta)
...
!> \param blk ...
! *****************************************************************************
  SUBROUTINE block_2d_chol_inv_c(m, blk)
    INTEGER, INTENT(IN)                      :: m
    COMPLEX(kind=real_4), INTENT(INOUT), DIMENSION(m,m)   :: blk

    CHARACTER(len=*), PARAMETER :: routineN = 'block_2d_chol_inv_c', &
      routineP = moduleN//':'//routineN
...
!   ---------------------------------------------------------------------------
    ...
    DO i=1,m
       ...
       ENDDO
    ENDDO
  END SUBROUTINE block_2d_chol_inv_c
...
! *****************************************************************************
  SUBROUTINE block_chol_inv_c(m, blk)
    INTEGER, INTENT(IN)                      :: m
    COMPLEX(kind=real_4), DIMENSION(m*m), INTENT(INOUT)   :: blk
...
!   ---------------------------------------------------------------------------
    ...
    CALL block_2d_chol_inv_c(m, blk)
