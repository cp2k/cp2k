...
void fftcu_plan3d_z(cufftHandle &plan, int &ioverflow, const int *n,
  ...
  ioverflow = 0;
  for (i = 0; i < n_plans; i++) {
    if (iplandims[i][0] == 3 && iplandims[i][1] == n[0] &&
        iplandims[i][2] == n[1] && iplandims[i][3] == n[2] &&
        iplandims[i][4] == 0) {
      plan = saved_plans[i];
      return;
    }
...
void fftcu_plan2dm_z(cufftHandle &plan, int &ioverflow, const int *n,
  ...
  ioverflow = 0;
  for (i = 0; i < n_plans; i++) {
    if (iplandims[i][0] == 2 && iplandims[i][1] == n[0] &&
        iplandims[i][2] == n[1] && iplandims[i][3] == n[2] &&
        iplandims[i][4] == fsign) {
      plan = saved_plans[i];
      return;
  ...
  if (VERBOSE)
    printf("FFT 2D (%d) (%d-%d-%d) %d %d %d %d\n", fsign, n[0], n[1], n[2],
           istride, idist, ostride, odist);
  cErr = cufftPlanMany(&plan, 2, nsize, inembed, istride, idist, onembed,
...
void fftcu_plan1dm_z(cufftHandle &plan, int &ioverflow, const int n,
  ...
  ioverflow = 0;
  for (i = 0; i < n_plans; i++) {
    if (iplandims[i][0] == 1 && iplandims[i][1] == n && iplandims[i][2] == m &&
        iplandims[i][3] == 0 && iplandims[i][4] == fsign) {
      plan = saved_plans[i];
      return;
    }
  }
  ...
  nsize[0] = n;
  inembed[0] = 0; // is ignored, but is not allowed to be NULL pointer (for adv.
                  // strided I/O)
  onembed[0] = 0; // is ignored, but is not allowed to be NULL pointer (for adv.
                  // strided I/O)
  batch = m;
  if (fsign == +1) {
    istride = m;
  ...
  if (VERBOSE)
    printf("FFT 1D (%d) (%d-%d) %d %d %d %d\n", fsign, n, m, istride, idist,
           ostride, odist);
  cErr = cufftPlanMany(&plan, 1, nsize, inembed, istride, idist, onembed,
                       ostride, odist, CUFFT_Z2Z, batch);
  if (CHECK)
    cufft_error_check(cErr, __LINE__);
  cErr = cufftSetStream(plan, cuda_stream);
