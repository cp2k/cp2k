...
// helper routine(s)
void get_grid_params(const int ngpts, const int blocksize,
...
extern "C" void pw_cuda_cfffg_z_(const double *din, cuDoubleComplex *zout,
  ...
  cErr = cudaMemcpyAsync(ptr_1, din, sizeof(double) * nrpts,
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  pw_copy_rc_cu_z<<<blocksPerGrid, threadsPerBlock, 0, cuda_streams[1]>>>(
      ptr_1, ptr_2, nrpts);
  cErr = cudaGetLastError();
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  ...
  cErr = cudaMemcpyAsync(ghatmap_dev, ghatmap, sizeof(int) * ngpts,
                         cudaMemcpyHostToDevice, cuda_streams[0]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaEventRecord(cuda_events[0], cuda_streams[0]);
  ...
  fftcu_run_3d_z_(+1, npts, 1.0e0, (cufftDoubleComplex *)ptr_2,
                  cuda_streams[1]);
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  pw_gather_cu_z<<<blocksPerGrid, threadsPerBlock, 0, cuda_streams[1]>>>(
      ptr_1, ptr_2, scale, ngpts, ghatmap_dev);
  cErr = cudaGetLastError();
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaMemcpyAsync(zout, ptr_1, sizeof(cuDoubleComplex) * ngpts,
                         cudaMemcpyDeviceToHost, cuda_streams[2]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
...
extern "C" void pw_cuda_sfffc_z_(const cuDoubleComplex *zin, double *dout,
  ...
  cErr = cudaMemcpyAsync(ptr_1, zin, sizeof(cuDoubleComplex) * ngpts,
                         cudaMemcpyHostToDevice, cuda_streams[0]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaMemcpyAsync(ghatmap_dev, ghatmap, sizeof(int) * nmaps * ngpts,
                         cudaMemcpyHostToDevice, cuda_streams[0]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaEventRecord(cuda_events[0], cuda_streams[0]);
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaMemsetAsync(
      ptr_2, 0, sizeof(double) * 2 * nrpts,
      cuda_streams[1]); // we need to do this only if spherical cut-off is used!
  if (CHECK)
    pw_cuda_error_check(
        cErr, __LINE__); // but it turns out to be performance irrelevant
  pw_scatter_cu_z<<<blocksPerGrid, threadsPerBlock, 0, cuda_streams[1]>>>(
      ptr_2, ptr_1, scale, ngpts, nmaps, ghatmap_dev);
  cErr = cudaGetLastError();
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  ...
  pw_copy_cr_cu_z<<<blocksPerGrid, threadsPerBlock, 0, cuda_streams[1]>>>(
      ptr_2, ptr_1, nrpts);
  cErr = cudaGetLastError();
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaMemcpyAsync(dout, ptr_1, sizeof(double) * nrpts,
                         cudaMemcpyDeviceToHost, cuda_streams[2]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
...
extern "C" void pw_cuda_cff_z_(const double *din, cuDoubleComplex *zout,
  ...
  // (cufftDoubleComplex *) ptr_1, cuda_streams[1]);
  fftcu_run_1dm_z_(1, npts[2], npts[0] * npts[1], 1.0e0,
                   (cufftDoubleComplex *)ptr_2, (cufftDoubleComplex *)ptr_1,
                   cuda_streams[1]);
  fftcu_run_1dm_z_(1, npts[1], npts[0] * npts[2], 1.0e0,
                   (cufftDoubleComplex *)ptr_1, (cufftDoubleComplex *)ptr_2,
                   cuda_streams[1]);
  cErr = cudaEventRecord(cuda_events[1], cuda_streams[1]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  // cErr = cudaMemcpyAsync(zout, ptr_1, sizeof(cuDoubleComplex) * nrpts,
  // cudaMemcpyDeviceToHost, cuda_streams[2]);
  cErr = cudaMemcpyAsync(zout, ptr_2, sizeof(cuDoubleComplex) * nrpts,
                         cudaMemcpyDeviceToHost, cuda_streams[2]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
...
extern "C" void pw_cuda_ffc_z_(const cuDoubleComplex *zin, double *dout,
  ...
  cErr = cudaMemcpyAsync(ptr_1, zin, sizeof(cuDoubleComplex) * nrpts,
                         cudaMemcpyHostToDevice, cuda_streams[0]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  cErr = cudaEventRecord(cuda_events[0], cuda_streams[0]);
  ...
  // (cufftDoubleComplex *) ptr_2, cuda_streams[1]);
  fftcu_run_1dm_z_(-1, npts[1], npts[0] * npts[2], 1.0e0,
                   (cufftDoubleComplex *)ptr_1, (cufftDoubleComplex *)ptr_2,
                   cuda_streams[1]);
  fftcu_run_1dm_z_(-1, npts[2], npts[0] * npts[1], 1.0e0,
  ...
  // pw_copy_cr_cu_z<<<blocksPerGrid, threadsPerBlock, 0,
  // cuda_streams[1]>>>(ptr_2, ptr_1, nrpts);
  pw_copy_cr_cu_z<<<blocksPerGrid, threadsPerBlock, 0, cuda_streams[1]>>>(
      ptr_1, ptr_2, nrpts);
  cErr = cudaGetLastError();
  if (CHECK)
  ...
  // cErr = cudaMemcpyAsync(dout, ptr_1, sizeof(double) * nrpts,
  // cudaMemcpyDeviceToHost, cuda_streams[2]);
  cErr = cudaMemcpyAsync(dout, ptr_2, sizeof(double) * nrpts,
                         cudaMemcpyDeviceToHost, cuda_streams[2]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
...
extern "C" void pw_cuda_f_z_(const cuDoubleComplex *zin, cuDoubleComplex *zout,
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  fftcu_run_1dm_z_(dir, n, m, 1.0e0, (cufftDoubleComplex *)ptr_1,
                   (cufftDoubleComplex *)ptr_2, cuda_streams[1]);
  cErr = cudaEventRecord(cuda_events[1], cuda_streams[1]);
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
...
extern "C" void pw_cuda_fg_z_(const cuDoubleComplex *zin, cuDoubleComplex *zout,
  ...
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
  fftcu_run_1dm_z_(1, npts[0], mmax, 1.0e0, (cufftDoubleComplex *)ptr_1,
                   (cufftDoubleComplex *)ptr_2, cuda_streams[1]);
...
extern "C" void pw_cuda_sf_z_(const cuDoubleComplex *zin, cuDoubleComplex *zout,
  ...
  // fft on the GPU (cuda_streams[1])
  fftcu_run_1dm_z_(-1, npts[0], mmax, 1.0e0, (cufftDoubleComplex *)ptr_2,
                   (cufftDoubleComplex *)ptr_1, cuda_streams[1]);
  cErr = cudaGetLastError();
  if (CHECK)
    pw_cuda_error_check(cErr, __LINE__);
