...
! **************************************************************************************************
  SUBROUTINE arnoldi_init_z (matrix, vectors, arnoldi_data)
   ...
   ! create a local data copy to store the vectors and make Gram Schmidt a bit simpler
    CALL cp_dbcsr_get_info(matrix=vectors%input_vec, nfullrows_local=nrow_local, nfullcols_local=ncol_local)
    ...
    IF(control%has_initial_vector)THEN
       ...
       CALL transfer_local_array_to_cp_dbcsr_z(vectors%input_vec, ar_data%f_vec, nrow_local, control%local_comp)
    ELSE
       ! Setup the initial normalized random vector (sufficient if it only happens on proc_col 0)
       CALL cp_dbcsr_iterator_start(iter, vectors%input_vec)
       DO WHILE (cp_dbcsr_iterator_blocks_left(iter))
          CALL cp_dbcsr_iterator_next_block(iter, row, col, data_vec, row_size=row_size, col_size=col_size)
    ...
    CALL transfer_cp_dbcsr_to_local_array_z(vectors%input_vec, v_vec, nrow_local, control%local_comp)
    ...
    CALL cp_dbcsr_scale(vectors%input_vec, REAL(1.0, real_8)/rnorm)
    ...
    DO i=1, SIZE(matrix)
       CALL cp_dbcsr_matrix_colvec_multiply(matrix(i)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                        CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
       CALL cp_dbcsr_copy(vectors%input_vec, vectors%result_vec)
    END DO

    CALL transfer_cp_dbcsr_to_local_array_z(vectors%result_vec, w_vec, nrow_local, control%local_comp)
...
! **************************************************************************************************
  SUBROUTINE build_subspace_z(matrix, vectors, arnoldi_data)
    ...
    TYPE(cp_dbcsr_type), POINTER                 :: input_vec, result_vec, swap_vec
    ...
    DO j=control%current_step, control%max_iter-1
       ...
       CALL transfer_local_array_to_cp_dbcsr_z(input_vec, v_vec, nrow_local, control%local_comp)
 
       ! This permits to compute the subspace of a matrix which is a product of two matrices
       DO i=1, SIZE(matrix)
          CALL cp_dbcsr_matrix_colvec_multiply(matrix(i)%matrix, input_vec, result_vec, CMPLX(1.0, 0.0, real_8), &
                                            CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
          swap_vec=>input_vec
       ...
       CALL transfer_cp_dbcsr_to_local_array_z(input_vec, w_vec, nrow_local, control%local_comp)
...
!> \param is_local ...
! **************************************************************************************************
  SUBROUTINE transfer_cp_dbcsr_to_local_array_z(vec, array, n, is_local)
    TYPE(cp_dbcsr_type)                          :: vec
    COMPLEX(kind=real_8), DIMENSION(:)           :: array
    INTEGER                                  :: n
    LOGICAL                                  :: is_local
    COMPLEX(kind=real_8), DIMENSION(:), POINTER          :: data_vec

    data_vec => cp_dbcsr_get_data_p (vec, select_data_type=CMPLX(0.0, 0.0, real_8))
    IF(is_local) array(1:n)=data_vec(1:n)

  END SUBROUTINE transfer_cp_dbcsr_to_local_array_z
...
!> \param is_local ...
! **************************************************************************************************
  SUBROUTINE transfer_local_array_to_cp_dbcsr_z(vec, array, n, is_local)
    TYPE(cp_dbcsr_type)                          :: vec
    COMPLEX(kind=real_8), DIMENSION(:)           :: array
    INTEGER                                  :: n
    LOGICAL                                  :: is_local
  ...
  END SUBROUTINE transfer_local_array_to_cp_dbcsr_z
...
! **************************************************************************************************
  ...
  SUBROUTINE gev_arnoldi_init_z (matrix, matrix_arnoldi, vectors, arnoldi_data)
    TYPE(cp_dbcsr_p_type), DIMENSION(:)     :: matrix
    TYPE(cp_dbcsr_p_type), DIMENSION(:)     :: matrix_arnoldi
    TYPE(m_x_v_vectors_type)                :: vectors
    TYPE(arnoldi_data_type)                 :: arnoldi_data
   ...
   ! create a local data copy to store the vectors and make Gram Schmidt a bit simpler
    ...
    CALL cp_dbcsr_matrix_colvec_multiply(matrix(1)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                      CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
    ...
    CALL cp_dbcsr_matrix_colvec_multiply(matrix(2)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                      CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
    ...
    CALL cp_dbcsr_copy(matrix_arnoldi(1)%matrix,matrix(1)%matrix)
    CALL cp_dbcsr_add(matrix_arnoldi(1)%matrix, matrix(2)%matrix, CMPLX(1.0, 0.0, real_8), -ar_data%rho_scale)
...
! **************************************************************************************************
  SUBROUTINE gev_build_subspace_z(matrix, vectors, arnoldi_data)
    ...
    CALL transfer_local_array_to_cp_dbcsr_z(vectors%input_vec, ar_data%x_vec, nrow_local, control%local_comp)
    CALL cp_dbcsr_matrix_colvec_multiply(matrix(2)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                      CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
    CALL transfer_cp_dbcsr_to_local_array_z(vectors%result_vec, BZmat(:,1), nrow_local, control%local_comp)
    ...
    DO j=1,control%max_iter
       ...
       CALL transfer_local_array_to_cp_dbcsr_z(vectors%input_vec, Zmat(:,j), nrow_local, control%local_comp)
       CALL cp_dbcsr_matrix_colvec_multiply(matrix(1)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                        CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
       CALL transfer_cp_dbcsr_to_local_array_z(vectors%result_vec, CZmat(:,j), nrow_local, control%local_comp)
       w_vec(:)=CZmat(:,j)                         
       ...
       CALL cp_dbcsr_matrix_colvec_multiply(matrix(2)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                        CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
       CALL transfer_cp_dbcsr_to_local_array_z(vectors%result_vec, v_vec, nrow_local, control%local_comp)
       norm=CMPLX(0.0, 0.0, real_8)
...
! compute convergence and set the correct eigenvalue and eigenvector
    ...
    ENDIF
    CALL transfer_local_array_to_cp_dbcsr_z(vectors%input_vec, v_vec, nrow_local, control%local_comp)
    CALL cp_dbcsr_matrix_colvec_multiply(matrix_arnoldi(1)%matrix, vectors%input_vec, vectors%result_vec, CMPLX(1.0, 0.0, real_8), &
                                            CMPLX(0.0, 0.0, real_8), vectors%rep_row_vec, vectors%rep_col_vec)
    CALL transfer_cp_dbcsr_to_local_array_z(vectors%result_vec, v_vec, nrow_local, control%local_comp)
    IF (ncol_local>0) THEN
       CALL compute_norms_z(v_vec, norm, rnorm, control%pcol_group)
