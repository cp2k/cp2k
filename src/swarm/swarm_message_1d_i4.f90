...
! *****************************************************************************
 SUBROUTINE swarm_message_add_1d_i4(msg, key, value)
   ...
   IF(swarm_message_haskey(msg, key)) &
      CALL mp_abort("swarm_message_add_1d_i4: key already exists: "//TRIM(key))
...
! *****************************************************************************
 SUBROUTINE swarm_message_get_1d_i4(msg, key, value)
   ...
   IF(ASSOCIATED(value)) CALL mp_abort("swarm_message_get_1d_i4: value already associated")

   curr_entry => msg%root
   DO WHILE(ASSOCIATED(curr_entry))
      IF(TRIM(curr_entry%key) == TRIM(key)) THEN
         IF(.NOT. ASSOCIATED(curr_entry%value_1d_i4)) &
            CALL mp_abort("swarm_message_get_1d_i4: value not associated key: "//TRIM(key))
