...
#include "grid_integrate_cuda.hu"
...
__device__ __inline__ void grid_prepare_pab_DADB(const int3 la, const int3 lb, struct pab_computation_struct_ &tp) {
...
__device__ __inline__ void grid_prepare_pab_ADBmDAB(const int3 la, const int3 lb, struct pab_computation_struct_ &tp) {
    // create a new pab_prep so that mapping pab_prep with pgf_a pgf_b
...
__device__ __inline__ void grid_prepare_pab_ARDBmDARB(const int3 la, const int3 lb, struct pab_computation_struct_ &tp) {
    // create a new pab_prep so that mapping pab_prep with pgf_a pgf_b
    // is equivalent to mapping pab with
...
__device__ __inline__ void grid_prepare_pab_DABpADB(const int3 la, const int3 lb, struct pab_computation_struct_ &tp) {
    // create a new pab_prep so that mapping pab_prep with pgf_a pgf_b
    // is equivalent to mapping pab with
    //    pgf_a (nabla_{idir} pgf_b) + (nabla_{idir} pgf_a) pgf_b
    // ( pgf_a ) (ddx pgf_b) + (ddx pgf_a)( pgf_b ) =
...
// *****************************************************************************
__device__ __inline__ void grid_prepare_pab_Di(const int3 la, const int3 lb, struct pab_computation_struct_ &tp) {
    ...
    case 'X': {
        ...
        jco_l = coset((lb.x + 1), lb.y, lb.z);
        tp.pab_prep(jco_l, ico_l) -=
            2.0 * la.x * tp.zeta[1] * pab;

        ico_l = coset((la.x + 1), la.y, la.z);
        jco_l = coset(max(lb.x - 1, 0), lb.y, lb.z);
        tp.pab_prep(jco_l, ico_l) -=
            2.0 * tp.zeta[0] * lb.x * pab;
    ...
    case 'Y': {
        ...
        jco_l = coset(lb.x, (lb.y + 1), lb.z);
        tp.pab_prep(jco_l, ico_l) -=
            2.0 * la.y * tp.zeta[1] * pab;

        ico_l = coset(la.x, (la.y + 1), la.z);
        jco_l = coset(lb.x, max(lb.y - 1, 0), lb.z);
        tp.pab_prep(jco_l, ico_l) -=
            2.0 * tp.zeta[0] * lb.y * pab;
    ...
    case 'Z': {
        ...
        jco_l = coset(lb.x, lb.y, (lb.z + 1));
        tp.pab_prep(jco_l, ico_l) -=
            2.0 * la.z * tp.zeta[1] * pab;

        ico_l = coset(la.x, la.y, (la.z + 1));
        jco_l = coset(lb.x, lb.y, max(lb.z - 1, 0));
        tp.pab_prep(jco_l, ico_l) -=
            2.0 * tp.zeta[0] * lb.z * pab;
...
// *****************************************************************************
__device__ __inline__ void oneterm_dijdij(const int idir, const double func_a, const int ico_l,
...
// *****************************************************************************
__device__ __inline__ void grid_prepare_pab_DiDj(const int3 la, const int3 lb, struct pab_computation_struct_ &tp) {
    // create a new pab_local so that mapping pab_local with pgf_a pgf_b
    // is equivalent to mapping pab with
    //   d_{ider1} pgf_a d_{ider1} pgf_b
    ...
    if ((tp.dir1 == 'X' && tp.dir2 == 'Y') ||
        ...
        func_a = 4.0 * tp.zeta[0] * tp.zeta[0] * pab;
        oneterm_dijdij(2, func_a, ico_l, lb.x, lb.y, lb.z, tp.zeta[1],
                       tp.pab_prep);
...
// *****************************************************************************
__device__ __inline__ void oneterm_diidii(const int idir, const double func_a, const int ico_l,
                                     const int lx, const int ly, const int lz,
                                     const double zet, Matrix  &pab_prep) {
    int jco_l;
...
// *****************************************************************************
__device__ __inline__ void grid_prepare_pab_Di2(const int3 la, const int3 lb, struct pab_computation_struct_  &tp) {
    // create a new pab_local so that mapping pab_local with pgf_a pgf_b
    // is equivalent to mapping pab with
...
__device__ void grid_prepare_pab(cg::thread_block &block, const int func, struct pab_computation_struct_ &tp) {
    int3 la, lb;
        ...
        for (lb.x = 0; lb.x <= tp.lmax[1]; lb.x++) {
            for (la.y = 0; la.y <= tp.lmax[0] - la.x; la.y++) {
                for (lb.y = 0; lb.y <= tp.lmax[1] - lb.x; lb.y++) {
                    ...
                    const int size_max = (tp.lmax[0] - la.x - la.y + 1) * (tp.lmax[1] - lb.x - lb.y + 1);
                    for (int i = i_min + block.thread_rank(); i < size_max; i += block.size()) {
                        ...
                        case GRID_FUNC_AB: {
                            const int ico = tp.offset[0] + coset(la.x, la.y, la.z);
                            const int jco = tp.offset[1] + coset(lb.x, lb.y, lb.z);
                            tp.pab_prep(coset(lb.x, lb.y, lb.z), coset(la.x, la.y, la.z)) = tp.pab(jco, ico);
                        }
                            break;
                        case GRID_FUNC_DADB:
                                             ...
                                             tensor4 &alpha)
{
    for(int iaxis = 0; iaxis < 3; iaxis++) {
        ...
        for (int i = block.thread_rank(); i < (lmax[0] + 1) *  (lmax[1] + 1); i += block.size()) {
            int lxa = i / (lmax[1] + 1);
            int lxb = i % (lmax[1] + 1);
...
__inline__ __device__ void prepare_coef_gpu(cg::thread_block &block,
                        ...
                        for (int lxa = lxa_min; lxa <= lmax[1] - lza - lya; lxa++) {
                            ...
                            for (int lzp = 0; lzp <= lza + lzb; lzp++) {
                                ...
                                for (int lxp = 0; lxp <= lp - lza - lzb; lxp++) {
                                    ...
                                    const double *__restrict__ const src = alpha.at(1, lyb, lya, 0);
                                    for (int lyp = block.thread_rank(); lyp <= lp - lza - lzb - lxp; lyp += block.size()) {
...
__global__ void compute_coefficients_gpu_(const _task *__restrict__ task_list_,
                                          const int2 lmin_diff,
                                          const int2 lmax_diff,
                                          const int *pab_offset_gpu_,
                                          double *const pab_gpu_,
                                          const int *__restrict__ coef_offset_gpu_,
                                          double *__restrict__ coef_gpu_)
{
    ...
    int lmin[2] = {
        max(task_list_[blockIdx.x].lmin[0] + lmin_diff.x, 0),
        max(task_list_[blockIdx.x].lmin[1] + lmin_diff.y, 0)
    };

    tensor4 alpha(3,
                  (task_list_[blockIdx.x].lmax[1] + 1),
                  (task_list_[blockIdx.x].lmax[0] + 1),
                  task_list_[blockIdx.x].l1_plus_l2_,
                  (double *)array);

    tensor3 coef(lmax[1] + lmax[0] + 1, lmax[1] + lmax[0] + 1, lmax[1] + lmax[0] + 1, lmax[1] + lmax[0] + 1, coef_gpu_ + coef_offset_gpu_[blockIdx.x]);
    prepare_alpha_gpu(block,
                      task_list_[blockIdx.x].ra,
                      task_list_[blockIdx.x].rb,
                      task_list_[blockIdx.x].rp,
                      lmax,
                      alpha);

    prepare_coef_gpu(block,
                     task_list_[blockIdx.x].prefactor,
                     lmin,
                     lmax,
                     alpha,
                     pab,
...
__global__ void compute_collocation_gpu_(
    ...
    const _task *__restrict__ task_list_,
    const int *__restrict__ coef_offset_gpu_,
    ...
    if (lmax == 0) {
        ...
        // memcpy_async(block, coefs_, coef, (((lmax + 1) * (lmax + 2) * (lmax + 3)) * sizeof(double)) / 6);
        for (int i = block.thread_rank();
             i < ((lmax + 1) * (lmax + 2) * (lmax + 3)) / 6; i += block.size())
            coefs_[i] = coef[i];
    }

    compute_cube_properties(radius, (double3*)task_list_[blockIdx.x].rp, &roffset, &cube_center,
                            &lb_cube, &cube_size);

    compute_window_size(&grid_size_,
                        &grid_lower_corner_pos_,
                        &period_, /* also full size of the grid */
                        task_list_[blockIdx.x].border_mask,
                        &border_width,
                        &window_size,
                        &window_shift);
...
extern "C" void compute_collocation_gpu(pgf_list_gpu *handler) {
    ...
    cudaMemcpyAsync(handler->task_list_gpu_,
                    handler->task_list_cpu_,
                    sizeof(_task) * handler->list_length,
    ...
    compute_collocation_gpu_<<<gridSize, blockSize, shared_mem,
        handler->stream>>>(
            ...
            handler->border_width, handler->task_list_gpu_,
