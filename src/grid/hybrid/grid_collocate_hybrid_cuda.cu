      ...
      for (int x = threadIdx.x; x <= cube_size.x; x += blockDim.x) {
        ...
        for (int alpha = 0; alpha <= lmax; alpha++) {
          ...
          for (int gamma = 0; gamma <= (lmax - alpha); gamma++) {
            ...
            for (int beta = 0; beta <= (lmax - alpha - gamma); beta++) {
#if __CUDA_ARCH__ < 600
              atomicAdd1(&coefs_[off + beta], exp_factor * dy);
#else
              atomicAdd(&coefs_[off + beta], exp_factor * dy);
#endif
              dy *= r3.y;
              off++;
            }
...
extern "C" void compute_collocation_gpu_variant(pgf_list_gpu *handler) {
  ...
  gridSize.x = handler->list_length;
  const int shared_mem = (handler->lmax + 1) * (handler->lmax + 2) *
                             (handler->lmax + 3) * sizeof(double) / 6 +
                         3 * handler->cmax * sizeof(int);
  compute_collocation_cube_gpu_<<<gridSize, blockSize, shared_mem,
                                  handler->stream>>>(
