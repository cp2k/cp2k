...
pgf_list_gpu *create_worker_list(const int number_of_workers,
                                 ...
                                 int device_id, const bool apply_cutoff);
...
static void initialize_worker_list_on_gpu(collocation_integration *handle,
    ...
    handle->worker_list =
        create_worker_list(number_of_workers, number_of_gaussian,
                           device_id, handle->apply_cutoff);
}
...
#pragma omp for schedule(static)
        for (int itask = 0; itask < ctx->tasks_per_level[level]; itask++) {
            ...
            add_orbital_to_list(current_worker, handler->coef.size[2] - 1, rp, task->radius,
                                zetp, &handler->coef);
        ...
        cublasDaxpy(handler->worker_list[0].blas_handle,
                    ...
                    &alpha,
                    handler->worker_list[1].data_gpu_, 1,
    ...
    if (ctx->number_of_devices == 1) {
        ...
        for (int thread = 1; thread < max_threads; thread++) {
            ...
            cublasDaxpy(ctx->handler[0]->worker_list[0].blas_handle, ctx->handler[0]->grid.alloc_size_,
                        &alpha, ctx->handler[thread]->worker_list[0].data_gpu_, 1,
...
/* two different handlers use the same gpu */
            ...
            const int thread_reduce = dev_thread[ctx->handler[thread]->worker_list[0].device_id];
            cublasDaxpy(ctx->handler[thread_reduce]->worker_list[0].blas_handle,
                        ctx->handler[thread_reduce]->grid.alloc_size_,
                        &alpha,
        ...
        for (int dev = 0; dev < ctx->number_of_devices; dev++) {
            ...
            cudaMemcpy(ctx->scratch, ctx->handler[thread_]->worker_list[0].data_gpu_, sizeof(double) * ctx->grid[level].alloc_size_, cudaMemcpyDeviceToHost);
            cblas_daxpy(ctx->grid[level].alloc_size_, 1.0, ctx->scratch, 1, ctx->grid[level].data, 1);
        }
    }
}
...
void grid_collocate_task_list_gpu(void *const ptr,
    ...
    /* need to set the grid parameters first i need the total size for allocating scratch memory */
    for (int level = 0; level < ctx->nlevels; level++) {
        set_grid_parameters(ctx, level,
                            npts_global[level], npts_local[level],
                            shift_local[level], border_width[level],
                            dh[level], dh_inv[level],
                            grid[level]);
    }
    ...
    */
    ...
    for (int thread = 0; thread < max_threads; thread++) {
        initialize_worker_list_on_gpu(ctx->handler[thread],
                                      ctx->device_id[thread % ctx->number_of_devices],
