...
void update_atoms_position(const int natoms, const double atoms_positions[natoms][3], grid_context *data)
{
    ...
    if (data->atom_positions == NULL) {
        ...
        if (natoms > data->natoms) {
            data->atom_positions = realloc(data->atom_positions, 3 * natoms * sizeof(double));
        }
    }
...
void allocate_buffer(const size_t buffer_size, grid_context *data)
{
    ...
    data->block_buffer_size_alloc = imax(data->block_buffer_size_alloc, buffer_size);
}
...
void update_basis_set(const int nkinds, const grid_basis_set **const basis_sets, grid_context *data) {
    if (nkinds > data->nkinds_total) {
        if (data->basis_sets == NULL) {
            ...
            data->basis_sets = realloc(data->basis_sets, nkinds * sizeof(grid_basis_set *));
        }
    }
    data->nkinds = nkinds;
...
void update_task_lists(const int nlevels, const int ntasks,
                       ...
                       const int *const iatom_list, const int *const jatom_list,
...
void *create_grid_context_cpu(
  ...
  update_task_lists(nlevels, ntasks,
                    level_list,
                    iatom_list, jatom_list,
                    iset_list,  jset_list,
                    ipgf_list,  jpgf_list,
  ...
  *blocks_buffer = ctx->blocks_buffer;
  ...
  ctx->handler = malloc(sizeof(struct collocation_integration_ *) * max_threads);
...
void set_grid_parameters(grid_context *ctx,
                         ...
                         double *grid_)
{
    ...
    initialize_tensor_3(&ctx->grid[grid_level],
                        grid_local_size[2],
                        grid_local_size[1],
                        grid_local_size[0]);
    ...
    if ((grid_local_size[0] != grid_full_size[0]) ||
        ...
        setup_grid_window(&ctx->grid[grid_level],
                          shift_local,
                          border_width,
    ...
    verify_orthogonality(ctx->grid[grid_level].dh, ctx->grid[grid_level].orthogonal);
...
void extract_grid_context_block_buffer(const void *const ptr, void *block_buffer)
{
    ...
    memcpy(block_buffer, ctx->blocks_buffer, sizeof(double) * ctx->block_buffer_size);
}
...
//#pragma omp parallel for
  for (int level = 0; level < ctx->nlevels; level++) {
      set_grid_parameters(ctx, level,
                          ...
                          grid[level]);
      memset(ctx->grid[level].data, 0, sizeof(double) * ctx->grid[level].alloc_size_);
  }
  ...
  for (int level = 0; level < ctx->nlevels; level++) {
      collocate_one_grid_level_dgemm(ctx, border_width[level], shift_local[level], func, level);
  }
