    ...
    for (int kg=kgmin; kg <= 1-kgmin; kg++) {
        ...
        for (int jg=jgmin; jg <= 1-jgmin; jg++) {
            ...
            for (int ig=igmin; ig<=1-igmin; ig++) {
                ...
                const int grid_index = k*npts_local[1]*npts_local[0] +
                                       j*npts_local[0] +
                                       i;
...
static void grid_collocate_core(const int lp,
    ...
    for (int kg=kgmin; kg <= 0; kg++) {
        ...
        for (int lzp=0; lzp <= lp; lzp++) {
            ...
            for (int lyp=0; lyp <= lp-lzp; lyp++) {
                for (int lxp=0; lxp <= lp-lzp-lyp; lxp++) {
                    coef_xy[lxy][0] += coef_xyz[lzp][lyp][lxp] * pol[2][lzp][kg -lb_cube[2]];
                    coef_xy[lxy][1] += coef_xyz[lzp][lyp][lxp] * pol[2][lzp][kg2-lb_cube[2]];
                    lxy++;
                }
                lxy += lzp;
            }
   ...
   // hence (0.0,0.0,0.0) in real space is rsgrid%lb on the rsgrid ((1,1,1) on grid)
    ...
    for (int i=0; i<3; i++) {
        ...
        if (npts_global[i] != npts_local[i]) {
            const int offset = modulo(cubecenter[i] + lb_cube[i] - shift_local[i], npts_global[i]) - lb_cube[i];
