...
!> \author  Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_cuda_hostmem_alloc_z (host_mem, n, error)
    COMPLEX(kind=real_8), DIMENSION(:), POINTER           :: host_mem
    INTEGER, INTENT(IN)                      :: n
    TYPE(dbcsr_error_type), INTENT(INOUT), OPTIONAL :: error
    TYPE(C_PTR)                              :: host_mem_c_ptr

    CALL dbcsr_cuda_hostmem_alloc_raw(host_mem_c_ptr, MAX(1,n)*(2*real_8_size), error)
#if defined (__DBCSR_CUDA)
    CALL C_F_POINTER (host_mem_c_ptr, host_mem, (/ MAX(1,n) /))
#else
    STOP "dbcsr_cuda_hostmem_alloc_z_4D: DBCSR_CUDA not compiled in."
#endif
  END SUBROUTINE dbcsr_cuda_hostmem_alloc_z
...
!> \author  Ole Schuett
! *****************************************************************************
SUBROUTINE dbcsr_cuda_hostmem_alloc_z_2D (host_mem, n1, n2, error)
    COMPLEX(kind=real_8), DIMENSION(:,:), POINTER           :: host_mem
    ...
    STOP "dbcsr_cuda_hostmem_alloc_z_4D: DBCSR_CUDA not compiled in."
#endif
  END SUBROUTINE dbcsr_cuda_hostmem_alloc_z_2D
...
!> \author  Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_cuda_hostmem_alloc_z_3D (host_mem, n1, n2, n3, error)
    COMPLEX(kind=real_8), DIMENSION(:,:,:), POINTER           :: host_mem
...
#else
    STOP "dbcsr_cuda_hostmem_alloc_z_3D: DBCSR_CUDA not compiled in."
#endif
  END SUBROUTINE dbcsr_cuda_hostmem_alloc_z_3D
...
!> \author  Ole Schuett
! *****************************************************************************
SUBROUTINE dbcsr_cuda_hostmem_alloc_z_4D (host_mem, n1, n2, n3, n4, error)
    COMPLEX(kind=real_8), DIMENSION(:,:,:,:), POINTER           :: host_mem
    ...
    STOP "dbcsr_cuda_hostmem_alloc_z_4D: DBCSR_CUDA not compiled in."
#endif
  END SUBROUTINE dbcsr_cuda_hostmem_alloc_z_4D
...
!> \author  Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_cuda_hostmem_dealloc_z (host_mem, error)
    COMPLEX(kind=real_8), DIMENSION(:), &
      POINTER                                :: host_mem
    TYPE(dbcsr_error_type), INTENT(INOUT), OPTIONAL :: error
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_cuda_hostmem_dealloc_z', &
    ...
    IF (istat /= 0 ) &
       STOP "dbcsr_cuda_hostmem_dealloc_z: Error deallocating host pinned memory"
#else
    STOP "dbcsr_cuda_hostmem_dealloc_z: DBCSR_CUDA not compiled in."
#endif
     IF (PRESENT (error)) CALL dbcsr_error_stop (error_handle, error)
  END SUBROUTINE dbcsr_cuda_hostmem_dealloc_z
...
!> \author  Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_cuda_hostmem_dealloc_z_2D (host_mem, error)
    COMPLEX(kind=real_8), DIMENSION(:,:), &
      POINTER                                :: host_mem
    TYPE(dbcsr_error_type), INTENT(INOUT), OPTIONAL :: error
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_cuda_hostmem_dealloc_z_2D', &
      routineP = moduleN//':'//routineN
    INTEGER                                  :: error_handle, istat
    ...
    IF (istat /= 0 ) &
       STOP "dbcsr_cuda_hostmem_dealloc_z_2D: Error deallocating host pinned memory"
#else
    STOP "dbcsr_cuda_hostmem_dealloc_z: DBCSR_CUDA not compiled in."
#endif
     IF (PRESENT (error)) CALL dbcsr_error_stop (error_handle, error)
  END SUBROUTINE dbcsr_cuda_hostmem_dealloc_z_2D
...
!> \author  Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_cuda_hostmem_dealloc_z_3D (host_mem, error)
    COMPLEX(kind=real_8), DIMENSION(:,:,:), &
      POINTER                                :: host_mem
    TYPE(dbcsr_error_type), INTENT(INOUT), OPTIONAL :: error
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_cuda_hostmem_dealloc_z_3D', &
      routineP = moduleN//':'//routineN
    INTEGER                                  :: error_handle, istat
    ...
    IF (istat /= 0 ) &
       STOP "dbcsr_cuda_hostmem_dealloc_z_3D: Error deallocating host pinned memory"
#else
    STOP "dbcsr_cuda_hostmem_dealloc_z: DBCSR_CUDA not compiled in."
#endif
     IF (PRESENT (error)) CALL dbcsr_error_stop (error_handle, error)
  END SUBROUTINE dbcsr_cuda_hostmem_dealloc_z_3D
...
!> \author  Ole Schuett
! *****************************************************************************
  SUBROUTINE dbcsr_cuda_hostmem_dealloc_z_4D (host_mem, error)
    COMPLEX(kind=real_8), DIMENSION(:,:,:,:), &
      POINTER                                :: host_mem
    TYPE(dbcsr_error_type), INTENT(INOUT), OPTIONAL :: error
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_cuda_hostmem_dealloc_z_4D', &
      routineP = moduleN//':'//routineN
    INTEGER                                  :: error_handle, istat
    ...
    IF (istat /= 0 ) &
       STOP "dbcsr_cuda_hostmem_dealloc_z_4D: Error deallocating host pinned memory"
#else
    STOP "dbcsr_cuda_hostmem_dealloc_z: DBCSR_CUDA not compiled in."
#endif
     IF (PRESENT (error)) CALL dbcsr_error_stop (error_handle, error)
  END SUBROUTINE dbcsr_cuda_hostmem_dealloc_z_4D
