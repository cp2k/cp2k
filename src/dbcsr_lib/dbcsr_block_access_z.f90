...
! *****************************************************************************
  SUBROUTINE dbcsr_get_2d_block_z(matrix,row,col,block,tr,found,&
       row_size, col_size)
    ...
    CHARACTER(len=*), PARAMETER :: routineN = 'dbcsr_get_2d_block_z', &
...
!> \param[out] col_size      (optional) logical column size of block
! *****************************************************************************
  SUBROUTINE dbcsr_get_block_z(matrix,row,col,block,tr,found,&
       row_size, col_size)
...
! *****************************************************************************
  SUBROUTINE dbcsr_get_tmp_block2d_z(matrix, row, col, block, tr,&
       found)
    TYPE(dbcsr_obj), INTENT(INOUT)           :: matrix
    INTEGER, INTENT(IN)                      :: row, col
    COMPLEX(kind=real_8), DIMENSION(:,:), POINTER         :: block
...
!$     __LINE__, error=error)
!$  iw = omp_get_thread_num () + 1
    CALL btree_get_z (matrix%m%wms(iw)%mutable%m%btree_z,&
         make_coordinate_tuple(stored_row, stored_col),&
         data_block, found)
...
!> \param[in,out] error        error
! *****************************************************************************
  SUBROUTINE dbcsr_set_block_pointer_1d_z (&
       matrix, pointer_any, row, col,&
       rsize, csize, main_tr, base_offset, buffer_tr, contiguous_pointers,&
       read_only, error)
    TYPE(dbcsr_obj), INTENT(IN)              :: matrix
    ...
    CHARACTER(len=*), PARAMETER :: &
      routineN = 'dbcsr_set_block_pointer_1d_z', &
      routineP = moduleN//':'//routineN
...
!   ---------------------------------------------------------------------------
    ...
    CALL dbcsr_get_data (matrix%m%data_area, pointer_any,&
         lb=base_offset, ub=base_offset+rsize*csize-1)
    IF (careful_mod) CALL dbcsr_error_stop (error_handler, error)
  END SUBROUTINE dbcsr_set_block_pointer_1d_z
