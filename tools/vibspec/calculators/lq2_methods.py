...
def calculate_lq2_spectrum_point(energy, state_idx, spectrum_type, oscillator_strengths, displacements,
                               frequencies, mode_count, alphas, vertical_energies, stokes, adiabatic_energies,
                               requested_states):
    """
    ...
    if spectrum_type == 'absorption':
        ...
        relaxation = _calculate_vibrational_relaxation(state_idx, mode_count, displacements,
                                                     frequencies, requested_states)
        energy_difference = (adiabatic_energies[state_idx - 1] - stokes / 2 - energy - relaxation)
    else:
    ...
    sigma = oscillator_strength * prefactor * np.exp(-(energy_difference ** 2) / (4 * alpha))
    return sigma


def _calculate_vibrational_relaxation(state_idx, mode_count, displacements, frequencies, requested_states):
    """Calculate vibrational relaxation energy"""
    state = requested_states[state_idx - 1]
    frequency_array = np.array([frequencies[j] for j in range(1, mode_count + 1)])
    displacement_array = np.array([displacements[state][j] for j in range(1, mode_count + 1)])
    return np.sum((displacement_array ** 2 * frequency_array) / 2)


def calculate_lq2_absorption(energy, state_idx, oscillator_strengths, displacements, frequencies, mode_count,
                           alphas, vertical_energies, stokes, requested_states):
    ...
    return calculate_lq2_spectrum_point(
        ...
        alphas, vertical_energies, stokes, None, requested_states
    )


def calculate_lq2_fluorescence(energy, state_idx, oscillator_strengths, displacements, frequencies, mode_count,
                             alphas, vertical_energies, stokes, adiabatic_energies, requested_states):
