...
class Cp2k(MakefilePackage, CudaPackage, CMakePackage, ROCmPackage):
    ...
    with when("+dlaf"):
        conflicts(
            "~mpi",
            msg="DLA-Future requires MPI. Only the distributed eigensolver is available.",
        )
    ...
    def fix_package_config(self):
        ...
        if self.spec.satisfies("@9.1:"):
            ...
            filter_file(
                r"(^includedir=).*",
                r"\1{0}".format(self.prefix.include),
                pkgconfig_file,
            )
            filter_file(
                r"(^libdir=).*", r"\1{0}".format(self.prefix.lib), pkgconfig_file
            )
            ...
            with open(pkgconfig_file, "r+") as handle:
                ...
                if (self.spec["fftw-api"].name == "fftw") and (
                    "+openmp" in self.spec["fftw"]
                ):
                    content += " -lfftw3_omp"
