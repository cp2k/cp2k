...
For periodic `PAW_ONE_CENTER`, `NATIVE_GRID_GAPW_COMPOSITE_GRID` selects the grid on which the
combined primitive fields are formed. `PERIODIC_ATOM_COMPOSITE`, the default, interpolates the
smooth field to GAPW radial/Lebedev grids and adds the hard-minus-soft fields there before
constructing the nonlinear Skala features. A periodic Becke-like partition over all atom images
