...
__global__ void
pw_gather_cu (float *pwcc, float *c, float scale, int *mapl, int *mapm,
	      int *mapn, int ngpts, int *nn, int *ghat, int l1, int l2,
	      int l3)
{
  ...
  for (igpt = 0; igpt < limit; igpt++)
    {
      ...
      ghattmp[threadIdx.x + NTHREADS] =
	ghat[index * 3 + threadIdx.x + NTHREADS];
      ghattmp[threadIdx.x + 2 * NTHREADS] =
	ghat[index * 3 + threadIdx.x + 2 * NTHREADS];
      __syncthreads ();
  ...
  if (threadIdx.x < nleftover)
    {
      ghattmp[threadIdx.x] = ghat[index * 3 + threadIdx.x];
      ghattmp[threadIdx.x + nleftover] =
	ghat[index * 3 + threadIdx.x + nleftover];
      ghattmp[threadIdx.x + 2 * nleftover] =
	ghat[index * 3 + threadIdx.x + 2 * nleftover];
    }
...
__global__ void
pw_scatter_cu (float *pwcc, float *c, float scale, int *mapl, int *mapm,
	       int *mapn, int ngpts, int *nn, int *ghat, int l1, int l2,
	       int l3)
{
...
extern "C" void
pw_fft_wrap_fg_cu_ (int *fsign, cufftComplex * zin, cufftComplex * zout,
		    float *scale, int *n, int *mapl, int *mapm, int *mapn,
		    int *ngpts, int *ghat, int *l1, int *l2, int *l3)
{
  ...
  cudaMemcpy (data, zin, sizeof (cufftComplex) * n[0] * n[1] * n[2],
  ...
  cudaMemcpy (ghat_dev, ghat, sizeof (int) * 3 * (*ngpts),
	      cudaMemcpyHostToDevice);
...
/*  cudaMemcpy(zin, data, sizeof(cufftComplex)*n[0]*n[1]*n[2], cudaMemcpyDeviceToHost);*/
  ...
  pw_gather_cu <<< NBLOCKS, NTHREADS >>> (ptr_out, ptr_in, *scale, mapl_dev,
					  mapm_dev, mapn_dev, *ngpts, n_dev,
					  ghat_dev, *l1, *l2, *l3);

  cudaThreadSynchronize ();

  cudaMemcpy (zout, pwcc, sizeof (cufftComplex) * n[0] * n[1] * n[2],
...
extern "C" void
pw_fft_wrap_sf_cu_ (int *fsign, cufftComplex * zin, cufftComplex * zout,
		    float *scale, int *n, int *mapl, int *mapm, int *mapn,
		    int *ngpts, int *ghat, int *l1, int *l2, int *l3)
{
  float *ptr_in, *ptr_out;
  ...
  cudaMemcpy (pwcc, zin, sizeof (cufftComplex) * n[0] * n[1] * n[2],
	      cudaMemcpyHostToDevice);
  cudaMemcpy (mapl_dev, mapl, sizeof (int) * n[0], cudaMemcpyHostToDevice);
  cudaMemcpy (mapm_dev, mapm, sizeof (int) * n[1], cudaMemcpyHostToDevice);
  cudaMemcpy (mapn_dev, mapn, sizeof (int) * n[2], cudaMemcpyHostToDevice);
  ...
  pw_scatter_cu <<< NBLOCKS, NTHREADS >>> (ptr_in, ptr_out, *scale, mapl_dev,
					   mapm_dev, mapn_dev, *ngpts, n_dev,
					   ghat_dev, *l1, *l2, *l3);
  ...
  cudaMemcpy (zout, data, sizeof (cufftComplex) * n[0] * n[1] * n[2],
	      cudaMemcpyDeviceToHost);
