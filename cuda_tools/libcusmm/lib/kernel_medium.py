...
class MediumKernel(object):
    def __init__(self, **params):
        self.__dict__.update(params)
        self.name  = "cusmm_kernel_medium_"
    ...
    def launcher_code(self):
       ...
       output += "cusmm_kernel_medium<%(m)d,%(n)d,%(k)d,%(tile_m)d,%(tile_n)d,%(panel_in)d,%(panel_out)d,%(grouping)d > "%self.__dict__
