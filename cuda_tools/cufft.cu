#if defined ( __FFTCU ) || defined ( __CUDAPW )
...
void
fftcu_plan3d (cufftHandle & plan, int *n, int &ioverflow)
{
  int i;
...
extern "C" void
fftcu_run_3d_cu_ (int *n, cufftComplex * data, int fsign, float scale)
{
  int ioverflow, lmem;
  cufftHandle plan;
...
extern "C" void
fftcu3d_cu_ (int *ifft_in_place, int *fsign, float *scale, int *n, float *zin,
	     float *zout)
{
  ...
  if (*ifft_in_place == 1)
    {
      cudaMemcpy (zin, data, sizeof (cufftComplex) * n[0] * n[1] * n[2],
...
extern "C" void
fftcu1dm_cu_ (int *fsign, int itrans, int *n, int *m, float *zin, float *zout,
	      float *scale)
{
  ...
  cudaMemcpy (data, zin, sizeof (cufftComplex) * (*n) * (*m),
	      cudaMemcpyHostToDevice);
  ...
  cudaMemcpy (zout, data, sizeof (cufftComplex) * (*n) * (*m),
	      cudaMemcpyDeviceToHost);
