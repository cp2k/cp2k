...
def test_remote_scf_status(h2_input, server_factory, ranks):
    with server_factory(h2_input, ranks=ranks) as (config, _):
        with SocketEnvironment(**config) as remote:
            assert remote.scf_converged is None
            assert remote.calculate().scf_converged is True
            assert remote.scf_converged is True
            remote.positions = remote.positions
            assert remote.scf_converged is None
    h2_input["FORCE_EVAL"]["DFT"]["SCF"].update(
        MAX_SCF=1,
        EPS_SCF=1e-30,
    ...
    with server_factory(h2_input, ranks=ranks) as (config, _):
        with SocketEnvironment(**config) as remote:
            assert remote.calculate(check_convergence=False).scf_converged is False
            assert remote.scf_converged is False
...
def test_remote_native(
    ...
    with server_factory(lj_input, ranks=ranks) as (config, process):
        with SocketEnvironment(**config) as remote:
            ...
            assert actual.scf_converged is None  # FIST has no Quickstep SCF.
