...
!> @file tblite/param/increment.f90
!> Provides a model for the core energy increment

!> Definition of the core energy increment
module tblite_param_increment
   ...
   !> Parametrization record for core energy increment
   type, public, extends(serde_record) :: increment_record
   contains
   ...
   !> Masking for the core energy increment
   type, public :: increment_mask
   end type increment_mask
...
subroutine load_from_toml(self, table, error)
   ...
   class(increment_record), intent(inout) :: self
   !> Data structure
...
subroutine dump_to_toml(self, table, error)
   !> Instance of the parametrization data
   class(increment_record), intent(in) :: self
   !> Data structure
   type(toml_table), intent(inout) :: table
   !> Error handling
   type(error_type), allocatable, intent(out) :: error
...
subroutine load_from_array(self, array, offset, base, mask, error)
   ...
   type(increment_record), intent(in) :: base
   type(increment_mask), intent(in) :: mask
   type(error_type), allocatable, intent(out) :: error

   select type(self)
   type is (increment_record)
      self = base
   end select
...
end module tblite_param_increment 
