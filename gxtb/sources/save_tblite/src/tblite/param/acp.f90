...
!> @file tblite/param/atomic_correction_potential.f90
!> Provides record for the atomic correction potential (ACP)

!> Defines model for the atomic correction potential (ACP)
module tblite_param_acp
   ...
   !> Parametrization model for the acp bonding
   type, public, extends(serde_record) :: acp_record
   contains
   ...
   !> Masking for the acp bonding model
   type, public :: acp_mask
   end type acp_mask
...
subroutine load_from_toml(self, table, error)
   ...
   class(acp_record), intent(inout) :: self
   !> Data structure
...
subroutine dump_to_toml(self, table, error)
   !> Instance of the parametrization data
   class(acp_record), intent(in) :: self
   !> Data structure
   type(toml_table), intent(inout) :: table
   !> Error handling
   type(error_type), allocatable, intent(out) :: error
...
subroutine load_from_array(self, array, offset, base, mask, error)
   ...
   type(acp_record), intent(in) :: base
   type(acp_mask), intent(in) :: mask
   type(error_type), allocatable, intent(out) :: error
