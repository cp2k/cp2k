...
module dftd4_model_d4srev
   ...
   use dftd4_model_reference_d4srev
   use dftd4_model_utils, only : is_exceptional, weight_cn, zeta_tanh, dzeta_tanh
   ...
   public :: d4srev_model, new_d4srev_model


   !> Revised D4S dispersion model to evaluate C6 coefficients
   type, extends(dispersion_model) :: d4srev_model
   ...
   character(len=*), parameter :: d4s_label = "D4Srev"

contains


!> Create new revised D4S dispersion model from molecular structure input
subroutine new_d4srev_model(error, d4, mol, qmod)
   !DEC$ ATTRIBUTES DLLEXPORT :: new_d4_model

   !> Instance of the dispersion model
   type(d4srev_model), intent(out) :: d4
   ...
   case(d4_qmod%eeq)
      do isp = 1, mol%nid
         ...
         call set_refalpha_eeq(d4%aiw(:, :, isp), izp)
      end do
      ! Setup EEQ model
   ...
   case(d4_qmod%eeqbc)
      do isp = 1, mol%nid
         ...
         call set_refalpha_eeqbc(d4%aiw(:, :, isp), izp)
      end do
      ! Setup EEQBC model
   ...
   case(d4_qmod%gfn2)
      do isp = 1, mol%nid
         ...
         call set_refalpha_gfn2(d4%aiw(:, :, isp), izp)
      end do
   case(d4_qmod%gxtb)
      do isp = 1, mol%nid
         izp = mol%num(isp)
         call set_refq_gxtb(d4%q(:, isp), izp)
         call set_refalpha_gxtb(d4%aiw(:, :, isp), izp)
      end do
   end select
...
end subroutine new_d4srev_model
...
subroutine update(self, mol, cache, cn, q, grad, only_c6)
   ...
   !> Instance of the dispersion model
   class(d4srev_model), intent(in) :: self
...
subroutine weight_references(self, mol, cn, q, gwvec, gwdcn, gwdq)
   ...
   !> Instance of the dispersion model
   type(d4srev_model), intent(in) :: self
   ...
   integer :: iat, izp, isp, iref, igw, jat, jzp
   real(wp) :: norm, dnorm, gw, expw, expd, gwk, dgwk, wf, maxcn, a, b, c, d
   ...
   if (present(gwdcn) .and. present(gwdq)) then
      ...
      !$omp private(iat, izp, isp, iref, igw, jat, jzp, a, b, c, d) &
      ...
      do iat = 1, mol%nat
         ...
         do jat = 1, mol%nat
            ...
            do iref = 1, self%ref(izp)
               ...
               gwvec(iref, iat, jat) = gwk * zeta_tanh(a, b, c, d, self%q(iref, izp), q(iat))
               gwdq(iref, iat, jat) = gwk * dzeta_tanh(a, b, c, d, self%q(iref, izp), q(iat))
               ...
               gwdcn(iref, iat, jat) = dgwk * zeta_tanh(a, b, c, d, self%q(iref, izp), q(iat))
            end do
      ...
      !$omp private(iat, izp, isp, iref, igw, a, b, c, d, jat, jzp) &
      !$omp private(norm, expw, gwk, wf, maxcn)
      do iat = 1, mol%nat
         izp = mol%id(iat)
         isp = mol%num(izp)
