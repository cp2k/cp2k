...
from typing import Any, Dict, Tuple, Optional, Protocol
...
# ======================================================================================
def run_matcher(output: str, matcher: str, **kwargs: Any) -> MatchResult:
    try:
        return registry[matcher].run(output, **kwargs)
    except:
        error = f'Matcher "{matcher}" failed:\n' + traceback.format_exc()
        return MatchResult(False, value=None, error=error)
...
class GenericMatcher(Matcher):
    ...
    def run(self, output: str, **kwargs: Any) -> MatchResult:
        ...
        if rel_error > tol:
            error = f"Difference too large: {rel_error:.2e} > {tol}, ref_value: {ref}, value: {value}."
            return MatchResult(False, value=value, error=error)

        return MatchResult(True, value=value, error=None)  # passed
